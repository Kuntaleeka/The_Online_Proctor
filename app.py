...
def capture_by_frames():
    ...
    while True:
        ...
        if not success or frame is None:
            ...
            continue  # Skip the empty frame and try again
...
def saveFaceInput():
    ...
    if utils.cap.isOpened():
        print("Releasing previous capture")
        utils.cap.release()

    # Attempt to access the camera
    cam = cv2.VideoCapture(0, cv2.CAP_DSHOW)  # Add CAP_DSHOW for DirectShow, which may help on Windows
    if not cam.isOpened():
        print("Error: Unable to open camera for capturing the profile photo.")
        return redirect(url_for('faceInput'))

    # Wait a bit to let the camera warm up
    time.sleep(1)  # Wait for 1 second to let the camera initialize

    # Capture multiple frames to ensure the camera is active
    for i in range(5):
        success, frame = cam.read()
        if not success:
            print(f"Error: Unable to capture frame {i+1} from the camera.")
            continue  # Try to capture the next frame

    # Check if we successfully captured a frame
    if not success or frame is None:
        print("Error: Unable to capture a valid frame from the camera.")
        return redirect(url_for('faceInput'))

    profileName = f"{studentInfo['Name']}_{utils.get_resultId():03}Profile.jpg"
    cv2.imwrite(f"static/Profiles/{profileName}", frame)  # Save the captured frame in the 'Profiles' folder
    print(f"Profile photo saved as {profileName}")

    # Move the saved profile photo to the designated folder if needed
