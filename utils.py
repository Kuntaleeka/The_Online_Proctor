...
end_time = [0, 0, 0, 0, 0]
recorded_durations = []
custom_dir = 'D:/The-Online-Exam-Proctor/vids'
...
capa.release()
video = [
    os.path.join(custom_dir, str(random.randint(1, 50000)) + ".mp4"),
    os.path.join(custom_dir, str(random.randint(1, 50000)) + ".mp4"),
    os.path.join(custom_dir, str(random.randint(1, 50000)) + ".mp4"),
    os.path.join(custom_dir, str(random.randint(1, 50000)) + ".mp4"),
    os.path.join(custom_dir, str(random.randint(1, 50000)) + ".mp4"),
...
f_name_directory = 'D:/The-Online-Exam-Proctor/auds'
# Capture
cap = None
...
def move_file_to_output_folder(file_name,folder_name='OutputVideo'):
    # Get the current working directory (project folder)
    current_directory = os.getcwd()
    # Define the paths for the source file and destination folder
    source_path = os.path.join(current_directory, 'vids', file_name)
...
def deleteTrashVideos():
    global video
    video_folder = 'D:/The-Online-Exam-Proctor/vids'
...
class FaceRecognition:
    ...
    def encode_faces(self):
        # Clear any existing encodings and names, if needed
        self.known_face_encodings = []
        self.known_face_names = []

        # Iterate over all images in the Profiles folder
        for image in os.listdir('static/Profiles'):
            image_path = f"static/Profiles/{image}"
            face_image = face_recognition.load_image_file(image_path)
            
            # Attempt to encode the face(s) in the image
            ...
            if face_encodings:
                ...
                print(f"Face encoding added for {image}")
            else:
                # If no face is detected, print a message
                print(f"No face detected in {image}, skipping this image.")
